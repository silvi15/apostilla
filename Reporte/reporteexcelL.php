<?php
	...
	$consulta = "SELECT * FROM apostillas WHERE fecha BETWEEN  '2017-01-01' AND '2017-01-31' AND intervencion='Legalizacion'";
	...
	if($resultado->num_rows > 0 ){
		...
		$titulosColumnas = array('N Tramite', 'Factura','Sr','Funcionario','Institución','Nombre Apellido',' Tipo Doc','Detalle','Arancel','Serie','Numero','Importe','fecha');
        ...
        while($fila2=$consulta2->fetch_array()){
			...
			$objPHPExcel->setActiveSheetIndex(0)
            		...
            		->setCellValue('E'.$i, $institucion)
            		->setCellValue('F'.$i, $nombreApellido)
            		->setCellValue('G'.$i, $nombreDoc)
            		->setCellValue('H'.$i, $titulardoc)
            		->setCellValue('I'.$i, $nombreArancel)
            		->setCellValue('J'.$i, $serie)
                    ->setCellValue('K'.$i, $numero)
                    ->setCellValue('L'.$i, $nombreImporte)
                    ->setCellValue('M'.$i, $fechaf);
					$i++;
		}
